...
const TrendingServices = ({ services = [] }) => {
  ...
  return (
    <section className="py-12 px-4 md:px-12">
      <h2 className="text-3xl font-bold text-center mb-8 dark:text-white">Trending Services</h2>
      {services.length > 0 ? (
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service) => (
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg overflow-hidden cursor-pointer transition-all duration-300 flex flex-col"
            >
              {service.image && (
                <img
                  src={service.image}
